...
def weight_load(model, path):
    whole_dict = torch.load(path)
    state = model.state_dict()
    print(state)
    state.update(whole_dict)
...
def save_model(save_dir, model, model_name, niter):
...
def save_opt(save_dir, opt, opt_name, niter):
    save_name = '{}_{}.pth'.format(opt_name, niter)
