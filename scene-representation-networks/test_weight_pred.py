...
p.add_argument('--weight_pred_checkpoint_path', default=None, help='Path to trained weight_pred.')
...
def test():
    ...
    weightPred = AnalogyPred(num_basis= 200, USE_EMBED=True, mode="regression").cuda()
    ...
    assert (opt.weight_pred_checkpoint_path is not None), "Have to pass checkpoint!"
    print("Loading weight pred model from %s" % opt.weight_pred_checkpoint_path)
    util.weight_load(weightPred, opt.weight_pred_checkpoint_path)
    weightPred.eval()
    weightPred.cuda()
    ...
    with torch.no_grad():
        ...
        for model_input, ground_truth in dataset:
            ...
            model_outputs = model(input = model_input, weighted_ebd = torch.matmul(feat_sim, basis_feat.squeeze()))
