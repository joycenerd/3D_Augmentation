...
p.add_argument('--steps_til_ckpt', type=int, default=2500,
...
def train():
    ...
    for img_sidelength, max_steps, batch_size in zip(img_sidelengths, max_steps_per_sidelength,
        ...
        print("The dataset len is ", len(train_dataset))
        # Loops over epochs.
        while True:
...
def save_mode_feature():
    ...
    with torch.no_grad():
        ...
        for i in range(len(dataset)):
            print("No:", i, " Obj_id: ", dataset[i][0][0]["instance_idx"])
            if (i+1) % 50 == 0:
                print("Avg features")
